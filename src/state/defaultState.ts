...
export const defaultState: GravyState = {
  ...
  todayBonusApplied: {},
  todayGameWins: 0,
  rollGoalAttemptsToday: 0,
  rollGoalTodayScore: 0,
  dayLogs: {},
  pendingRewards: [],
  pendingPointsAwards: [],
  actionLog: [],
...
function sanitizeState(state: GravyState): void {
  ...
  state.rollGoalAttemptsToday = asFiniteNumber(state.rollGoalAttemptsToday, 0);
  state.rollGoalTodayScore = asFiniteNumber(state.rollGoalTodayScore, 0);
...
export function applyDayRollover(state: GravyState): GravyState {
  ...
  if (state.lastActiveDate && state.lastActiveDate !== today) {
    ...
    state.rollGoalAttemptsToday = 0;
    state.rollGoalTodayScore = 0;
  }
