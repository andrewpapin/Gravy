...
describe('applyDayRollover', () => {
  ...
  it('resets per-day ledgers (todayGoalCounts, todayBonusApplied, todayGameWins, rollGoalAttemptsToday, rollGoalTodayScore) for the new day', () => {
    setToday('2024-01-11T08:00:00Z');
    const state = freshState({
      lastActiveDate: '2024-01-10',
      todayPoints: 10,
      ...
      todayGameWins: 3,
      rollGoalAttemptsToday: 3,
      rollGoalTodayScore: 950,
    });
    const result = applyDayRollover(state);
    expect(result.todayGoalCounts).toEqual({});
    expect(result.todayBonusApplied).toEqual({});
    expect(result.todayGameWins).toBe(0);
    expect(result.rollGoalAttemptsToday).toBe(0);
    expect(result.rollGoalTodayScore).toBe(0);
  });
